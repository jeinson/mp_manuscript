...
### MAIN SCRIPT
def main():
    vep_cons = np.loadtxt("/gpfs/commons/groups/lappalainen_lab/jeinson/data/vep_severity_ext.txt", dtype = "str", encoding = None)[:,0]
    ...
    tabix_gnomad = pysam.Tabixfile(args.gnomad, "r")
    ...
    if recompute == True:
        ...
        exons = df_eqtl[(df_eqtl.chr == args.chrom) & (df_eqtl.padj < .05)].pid
    ...
    for line in vcf_coding: # Loop through all CODING variants
        ...
        if line[0:1] != "#" and ('0|1' in line or '1|0' in line):
            ...
            gnom = tabix_gnomad.fetch(coding_cols[0], int(coding_cols[1]) - 1, int(coding_cols[1]))
                        
            # Change here: some variants in TOPMED aren't in Gnomad
            gnomad_af='NA'
            try:
                for gnom_rec in gnom:
                    if (gnom_rec.split("\t")[3] == coding_cols[3]) & (gnom_rec.split("\t")[4] == coding_cols[4]): 
                        gnomad_af = fun_dict_from_vcf_info(gnom_rec.split("\t")[7])['AF']
                        break
            except:
                continue
            ...
            # Note in Topmed, the annotations are labeled as 'ANN' in the VCF, instead of 'CSQ'
            if 'ANN' in info_dict:
                veps = info_dict['ANN'].split(";")
                ...
                for vep in veps:
                    ...
                    # Note this is also formatted slightly differently than the GTEx VCFs
                    ...
                    #for exon in np.unique(exons):
                        ...
                        if (gene in list(df_eqtl.group[df_eqtl.padj < .05])):
                            ...
                            f = open("/gpfs/commons/groups/lappalainen_lab/dglinos/projects/epistasis/TOPMED/names.txt", 'r')
                            indvs = f.read().split()
                            f.close()
                            ...
                            esnp_af = float(phased_cols[7].split(";")[3].split("=")[1])
                            #import code; code.interact(local=locals()); sys.exit()
                            ...
                            # output downstream
                            csnp_id = "_".join([csnp_geno[i] for i in [0,1,3,4]])
                            esnp_id = "_".join([eqtl_geno[i] for i in [0,1,3,4]])
                            def print_line(indv, hap):
                                flush_print("\t".join(map(str, [
                                    # Sample Info
                                    indv, gene, egene_row['pid'], egene_row['top_pid'], egene_row['chr'],
                                    #cSNP info
                                        csnp_id, cons_out, cadd_effect, cadd,
                                        var_af, gnomad_af, var_type, csnp_ref, csnp_alt, anc_allele, "-",
                                    #QTL SNP info
                                        esnp_id, qtl_hi_inc_allele, qtl_lo_inc_allele, qtl_anc_allele,
